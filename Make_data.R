...
TZ_canopy_height <- raster('TZ_GEDI_500m.tif') %>% mask(., TZ_outline) 
TZ_ann_rain_2000s <- raster('TZ_annual_median_rain_00_20.tif') %>% mask(., TZ_outline) %>% projectRaster(., TZ_canopy_height)
TZ_min_temp_2000s <- raster('TZ_MODIS_coldest_temperature_00_20.tif') %>% mask(., TZ_outline) %>% projectRaster(., TZ_canopy_height)
TZ_max_temp_2000s <- raster('TZ_MODIS_hottest_temperature_00_20.tif') %>% mask(., TZ_outline) %>% projectRaster(., TZ_canopy_height)
TZ_population <- raster('TZ_worldpop_2020_500m.tif') %>% mask(., TZ_outline) %>% projectRaster(., TZ_canopy_height)
degind_2010_14 <- raster('BG_2010_14_500m.tif') %>% mask(., TZ_outline) %>% projectRaster(., TZ_canopy_height)

setwd('/Users/philism/OneDrive - NTNU/PhD/Joris_work/Temporal_variables')

TZ_annual_median_rain_00_20 <- raster('TZ_annual_median_rain_00_20.tif') %>% mask(., TZ_outline) %>% projectRaster(., TZ_canopy_height)
TZ_annual_median_rain_60_90 <- raster('TZ_annual_median_rain_60_90.tif') %>% mask(., TZ_outline) %>% projectRaster(., TZ_canopy_height)
...
##REMOVED degind_2010_14 and indicator
variables <- stack(TZ_ann_rain_2000s, TZ_min_temp_2000s, TZ_max_temp_2000s, TZ_population, TZ_canopy_height)
variables <- as(variables, "SpatialPointsDataFrame")
##REMOVED "BG" and "indicator"
names(variables) <- c("rain", "temp_min", "temp_max", "pop",  "canopy")
...
setwd('/Users/philism/OneDrive - NTNU/PhD/Joris_work/Philip_data')
save(proj, TZ_outline, ebird_full, atlas_full, Mesh, stk.ip, stk.pred, variables_no_BG, file = paste0("TZ_INLA_model_file_E", round(max.edge, digits = 3), ".RData"))
